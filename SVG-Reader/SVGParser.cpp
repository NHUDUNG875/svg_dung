...
void SVGParser::parseAttributes(tinyxml2::XMLElement* xmlNode, SVGElement* element) {
    ...
    while (attr) {
        ...
        // --- SỬA LỖI TẠI ĐÂY: Attribute stroke="none" ---
            ...
            if (s) {
                if (name == "stroke") {
                    if (value == "none") s->strokeOpacity = 0.0f;
                    else s->strokeColor = CustomColor::fromStringToCustomColor(value);
                }
        ...
        // -------------------------------------------------
            ...
            if (value == "italic") currentStyle.setFontStyle(currentStyle.getFontStyle() | 2);
        }
        else if (name == "font-weight") {
            if (value == "bold" || value == "bolder" || value == "700") currentStyle.setFontStyle(currentStyle.getFontStyle() | 1);
        }
        else if (name == "fill-rule") { // Thêm fill-rule nếu cần
            currentStyle.setFillRule(value);
        }
        attr = attr->Next();
    }
...
void SVGParser::parseStroke(Stroke& stroke, const std::string& styleStr) {
    ...
    // --- SỬA LỖI TẠI ĐÂY: Style stroke:none ---
    if (key == "stroke") {
        if (value == "none") stroke.strokeOpacity = 0.0f;
        else stroke.strokeColor = CustomColor::fromStringToCustomColor(value);
    }
    // ------------------------------------------
