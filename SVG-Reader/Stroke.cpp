...
Stroke::Stroke(const Stroke& other)
    ...
    miterLimit(other.miterLimit)//them 
{}
...
Stroke& Stroke::operator=(const Stroke& other) {
    if (this != &other) {
        ...
        this->miterLimit = other.miterLimit;// them 
    }
    return (*this);
}
Stroke::~Stroke() {}
...
Gdiplus::Color Stroke::getGdiColor() const {
    ...
    Gdiplus::Color baseColor = strokeColor; 
...
void Stroke::parse(const std::string& strokeStr) {
    ...
    if (key == "stroke") this->strokeColor = CustomColor::fromStringToCustomColor(value);
        ...
        try { this->strokeWidth = std::stof(value); }
        catch (...) {}
    }
    else if (key == "stroke-opacity") {
        try { this->strokeOpacity = std::stof(value); }
        catch (...) {}
    }
    // (Lưu ý: stroke-miterlimit thường được parse ở SVGParser::parseAttributes chứ ít khi nằm trong chuỗi stroke rút gọn, 
    // nhưng nếu có thì thêm vào đây cũng được)
    else if (key == "stroke-miterlimit") {
        try { this->miterLimit = std::stof(value); }
        catch (...) {}
    }
}
//void Stroke::parse(const std::string& strokeStr) {
